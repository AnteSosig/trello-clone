...
/**
 * Check if user has required permission based on role for project operations
 */
int check_permission(const AuthContext* auth, Permission required_permission) {
    ...
    // Define role-based permissions for project service
    if (strcmp(auth->role, "MANAGER") == 0) {
        ...
        switch (required_permission) {
            case PERM_READ_PROJECTS:
                return 0; // Users can read projects they're members of
            case PERM_CREATE_PROJECTS:
            case PERM_UPDATE_PROJECTS:
            case PERM_DELETE_PROJECTS:
            case PERM_MANAGE_MEMBERS:
