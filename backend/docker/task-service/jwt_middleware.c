...
/**
 * Check if user has required permission based on role for task operations
 */
int check_permission(const AuthContext* auth, Permission required_permission) {
    ...
    // Define role-based permissions for tasks service
    if (strcmp(auth->role, "MANAGER") == 0) {
        ...
        switch (required_permission) {
            case PERM_READ_TASKS:
                return 0; // Users can read tasks they're assigned to or in projects they're members of
            case PERM_CREATE_TASKS:
            case PERM_UPDATE_TASKS:
            case PERM_DELETE_TASKS:
            case PERM_ASSIGN_TASKS:
